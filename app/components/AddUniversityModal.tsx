...
const AddUniversityModal = ({ isOpen, onClose, onAdd }: AddUniversityModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const newUniversity = await organizationAPI.universities.create({
      ...
      setSubmitError(error.response?.data?.error || 'Failed to create university. Please try again.')
    } finally {
    ...
    >
      ...
      >
        <div className="sticky top-0 bg-main border-b border-custom px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between rounded-t-xl z-10">
          <h2 className="text-xl sm:text-2xl font-bold text-brand pr-2">Add New University</h2>
          <button
            onClick={handleClose}
