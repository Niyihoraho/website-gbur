...
import { organizationAPI, RegionalStaff, Region } from '../lib/api'

interface AddRegionalStaffModalProps {
  isOpen: boolean
  onClose: () => void
  onAdd: (staff: RegionalStaff) => void
}

const AddRegionalStaffModal = ({ isOpen, onClose, onAdd }: AddRegionalStaffModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const regionIdNum = parseInt(formData.regionId, 10)
      if (isNaN(regionIdNum)) {
        setSubmitError('Invalid region selected')
        return
      }

      const newStaff = await organizationAPI.regionalStaff.create({
      ...
      console.error('Error adding regional staff:', error)
      console.error('Error response:', error.response?.data)
      const errorMessage = error.response?.data?.error || error.response?.data?.message || error.message || 'Failed to create regional staff. Please try again.'
    ...
    >
      ...
      >
        <div className="sticky top-0 bg-main border-b border-custom px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between rounded-t-xl z-10">
          <h2 className="text-xl sm:text-2xl font-bold text-brand pr-2">Add Regional Staff</h2>
          <button
            onClick={handleClose}
        ...
        <div className="p-4 sm:p-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <label htmlFor="phone" className="block text-xs sm:text-sm font-semibold text-brand mb-2">
                Phone <span className="text-xs text-secondary font-normal">(Optional)</span>
              </label>
              <input
                type="text"
                id="phone"
...
export default AddRegionalStaffModal
