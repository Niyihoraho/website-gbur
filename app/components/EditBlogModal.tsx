...
const EditBlogModal = ({ isOpen, onClose, post, onUpdate, onRefresh }: EditBlogModalProps) => {
  ...
  useEffect(() => {
    if (isOpen && post) {
      setFormData({
        ...
        image: post.image || '',
        excerpt: Array.isArray(post.excerpt) ? post.excerpt.join('\n') : post.excerpt || '',
  ...
  useEffect(() => {
    if (post && categories.length > 0) {
      // Find category by name (since we store category as name in BlogPost)
      const category = categories.find(cat => cat.name.toUpperCase() === post.category)
      if (category) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const blogData: Partial<CreateBlogPostInput> = {
        title: formData.title.trim(),
        slug: slug !== post.slug ? slug : post.slug, // Always send slug, but use existing if unchanged
        content,
      ...
      if (formData.status === 'published') {
        ...
        // If already published, keep existing publishedAt (don't send it)
      } else {
        // If changing to draft/archived, clear publishedAt
        blogData.publishedAt = null
      }

      // Update the blog post via API
      const updatedPost = await blogAPI.update(post.id, blogData)
      ...
      setSubmitError(error.response?.data?.error || 'Failed to update blog post. Please try again.')
    } finally {
      setIsSubmitting(false)
    }
  }
    ...
    >
      ...
      >
        ...
        <div className="sticky top-0 bg-main border-b border-custom px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between rounded-t-xl z-10">
          <h2 className="text-xl sm:text-2xl font-bold text-brand pr-2">Edit Blog Post</h2>
          <button
            onClick={handleClose}
