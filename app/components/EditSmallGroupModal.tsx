...
interface EditSmallGroupModalProps {
  ...
  onUpdate: (smallGroup: SmallGroup) => void
}

const EditSmallGroupModal = ({ isOpen, onClose, smallGroup, onUpdate }: EditSmallGroupModalProps) => {
  ...
  useEffect(() => {
    if (isOpen && smallGroup) {
      setFormData({
        name: smallGroup.name || '',
        type: smallGroup.type || 'student',
        province: smallGroup.province || '',
        district: smallGroup.district || '',
        sector: smallGroup.sector || '',
        address: smallGroup.address || '',
        cellLeaderName: smallGroup.cellLeaderName || '',
        WhatappNumber: smallGroup.WhatappNumber || '',
      })
      setErrors({})
      setSubmitError(null)
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updatedSmallGroup = await organizationAPI.smallGroups.update(smallGroup.id, {
      ...
      onUpdate(updatedSmallGroup)
      onClose()
    } catch (error: any) {
      console.error('Error updating small group:', error)
      setSubmitError(error.response?.data?.error || 'Failed to update small group. Please try again.')
    } finally {
  ...
  if (!isOpen || !smallGroup) return null
    ...
    >
      ...
      >
        <div className="sticky top-0 bg-main border-b border-custom px-4 sm:px-6 py-3 sm:py-4 flex items-center justify-between rounded-t-xl z-10 flex-shrink-0">
          <h2 className="text-xl sm:text-2xl font-bold text-brand pr-2">Edit Small Group</h2>
          <button
            onClick={handleClose}
...
export default EditSmallGroupModal
